...
async function getStorage() {
  if (!storage) {
    const settingsRes = await fetch(new URL('/api/settings', process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : 'http://localhost:3000'));
    if (settingsRes.ok) {
      const data = await settingsRes.json();
      storage = { cookie: data.cookie, filters: data.filters };
    }
  }
  return storage;
}
...
async function getStats() {
  if (!stats) {
    const statsRes = await fetch(new URL('/api/stats', process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : 'http://localhost:3000'));
    if (statsRes.ok) {
...
async function updateStats(data: any) {
  await fetch(new URL('/api/stats', process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : 'http://localhost:3000'), {
    method: 'POST',
...
export async function POST(request: Request) {
  try {
    ...
    const cookie = storageData?.cookie;
    ...
    await updateStats({ pending_requests: friendRequests.length });
    ...
    const totalDeclined = (currentStats?.totalDeclined || 0) + declined;
    await updateStats({
      total_declined: totalDeclined,
      last_run: new Date().toLocaleString(),
      pending_requests: friendRequests.length - declined
    });
